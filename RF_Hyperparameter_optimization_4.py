...
kde_1_figname = 'RF_KDE_plot_1.png'
kde_2_figname = 'RF_KDE_plot_2.png'
lc_figname = 'RF_Learning_Curve.png'
...
def main():
    ...
    print('Accuracy score of the best RF model: {}'.format(Score))
    ...
    print('ROC accuracy on traing set: {}'.format(roc_auc_score(y_train, best_RF_model.predict_proba(X_train)[:,1])))
    print('ROC accuracy on test set: {}'.format(roc_auc_score(y_test, best_RF_model.predict_proba(X_test)[:,1])))

    # Kernel density plot for train and validation set
    plot_kde_1(best_RF_model, X_train, y_train, skf, kde_1_figname)
    plot_kde_2(best_RF_model, X_train, y_train, skf, kde_2_figname)

    plot_lc(best_RF_model, X_train, y_train, skf, lc_figname)
